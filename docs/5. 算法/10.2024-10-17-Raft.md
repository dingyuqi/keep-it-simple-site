...
![图 1: 复制状态机架构](/screen_shot/replicated-state-machine.png =400x)
...
![图 2: Raft 节点状态转换](/screen_shot/raft-role-change.png =600x)
...
![图 3: Raft 任期](/screen_shot/raft-term.png =500x)

### 日志复制
![图 4: Raft 日志复制](/screen_shot/raft-log-replication.png =500x)
...
![图 5: Follower 的日志可能会与新 Leader 的日志不一致的情况](/screen_shot/raft-log-replication-error.png =500x)
...
![图 6:交先前任期的日志条目](/screen_shot/raft-commit-entries-from-previous-terms.png =500x)
...
- Raft 通过无限重试来处理这些失败. 如果崩溃的服务器重新启动, RPC将成功完成. 如果服务器在完成 RPC 之后但在响应之前崩溃, 那么它将在重新启动后再次接收相同的RPC.
- 如果一个 Follower 接收到一个 AppendEntries请求, 该请求包含已经在其日志中存在的日志条目, 则它将忽略新请求中的那些条目. ==即 Raft 中的 RPC 是幂等的.==
...
$$
- broadcastTime: 节点向集群中每个节点并行发送 RPC 并接受响应所需的平均时间. 0.5ms-20ms.
- electionTimeout: 选举超时. 10ms-500ms.
